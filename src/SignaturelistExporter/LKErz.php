...
class LKErz extends Base
{
    ...
    public function generateExport(array $fields, string $filename, array $options)
    {
        ...
        $spreadsheet = $reader->load(__DIR__ . "/../../assets/signaturepresets/lk-erz.xlsx");
        ...
        foreach ($anmeldeListe as $groupTitle => $anmeldungen) {
            ...
            $activeWorksheet->mergeCells("H9:I9");
            $activeWorksheet->setCellValue("H9", $this->ruestzeit->getDateFrom()->format("d.m.Y") ." - " . $this->ruestzeit->getDateTo()->format("d.m.Y"));
            ...
            foreach ($anmeldungen as $index => $anmeldung) {
                ...
                if($anmeldung->getPersonenTyp() == PersonenTyp::MITARBEITER || $anmeldung->getPersonenTyp() == PersonenTyp::REFERENT) {
                    ...
                    $activeWorksheet->setCellValue("D" . $rowIndex, $anmeldung->getAge());
                    $activeWorksheet->setCellValue("F" . $rowIndex, $anmeldung->getPostalcode() . " " . $anmeldung->getCity());

                    $activeWorksheet->setCellValue("G" . $rowIndex, $this->ruestzeit->getDateFrom()->format("d.m."));
                    $activeWorksheet->setCellValue("H" . $rowIndex, $this->ruestzeit->getDateTo()->format("d.m."));

                    $rowIndex += 1;

                    // We can only have 5 leaders
                    if($rowIndex == 19) break;
                }
            }
